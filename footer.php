<!-- sustainable section desktop -->
<section class="sustain-section">
  <img
    src="assets/icons/images/Hållbarhet 1.png"
    class="sustain-desktop-img" />
  <div class="sustain-content">
    <p class="sustain-desktop-text">INNOVATION SOM MINIMERAR AVTRYCKET</p>
    <p class="sustain-desktop-text-right">
      FUNKTION SKAPAD AV SMARTA MATERIAL
    </p>
    <p class="sustain-mobile-text">
      Innovation som minimerar avtrycket, funktion skapad av smarta material
    </p>
    <h2 class="sustain-header">
      DET ÄR <span class="sustain-sustain-header">HÅLLBARHET.</span>
    </h2>
    <button class="sustain-button button-animation">
      vår teknik
      <svg class="icon">
        <use href="assets/icons/icons.svg#icon-see-more"></use>
      </svg>
    </button>
  </div>
</section>

<section class="Urban-performance-section">
  <h2> <span class="h2-span">URBAN PERFORMANCE</span></h2>
  <img
    src="/assets/icons/images/Urban-performance-mobile 1 (1).png"
    class="img-mobile" />
  <img src="/assets/icons/images/Frame 417.jpg" class="img-desktop" />
</section>

<!-- mobile the drop -->
<section class="the-drop">
  <img src="assets/icons/images/Bild-blurred-1.png" alt="Background" />
  <img src="assets/icons/images/Eye 2.png" alt="Eye Icon" />

  <div class="date-container">
    <div class="images">
      <img src="assets/icons/images/Subtract.png" alt="Icon 1" />
      <img src="assets/icons/images/Subtract1.png" alt="Icon 2" />
    </div>
    <p>DAGAR KVAR</p>
    <button class="desktop-drop-button  button-animation">
      Stay tuned
      <svg class="icon">
        <use href="assets/icons/icons.svg#icon-see-more"></use>
      </svg>
  </div>

</section>

<!-- desktop the drop-->
<section class="desktop-container">
  <div class="desktop-left">
    <img
      src="assets/icons/images/Bild+öga 1.png"
      class="desktop-drop-left" />
    <div class="desktop-drop-text">
      <p>
        Här bakom döljer sig vårt nästa släpp. Vi finjusterar det sista.
      </p>
    </div>
  </div>
  <section class="desktop-drop-right">
    <h2 class="desktop-drop-title">THE DROP</h2>

    <div class="desktop-drop-countdown">
      <div class="countdown-number"><img src="assets/icons/images/date1.png"></div>
      <div class="countdown-number"><img src="assets/icons/images/date2.png"></div>
    </div>

    <p class="desktop-drop-label">DAGAR KVAR</p>

    <button class="desktop-drop-button  button-animation">
      Skapa din design
      <svg class="icon">
        <use href="assets/icons/icons.svg#icon-see-more"></use>
      </svg>
    </button>
  </section>
</section>

<div class="container">
  <h2> <span class="h2-span">VI ÄR KINFORMA</span></h2>

  <div class="carousel-container">
    <div class="carousel">
      <div class="card">
        <img src="assets/icons/images/Comback.Bag.back.MVP.png" alt="" />
      </div>
      <div class="card">
        <img src="assets/icons/images/Comback.Bag.front.MVP.png" />
      </div>
      <div class="card">
        <img
          src="assets/icons/images/Kinforma_collar&hooded_greyjacket_1.png"
          alt="" />
      </div>
      <div class="card">
        <img src="assets/icons/images/Kinforma_model_bag.png" alt="" />
      </div>
      <div class="card">
        <img
          src="assets/icons/images/Termos.preview.flera.MVP.png"
          alt="" />
      </div>
      <div class="card">
        <img src="assets/icons/images/Kopia av SUNGLASSES.png" alt="" />
      </div>
      <div class="card">
        <img src="assets/icons/images/Comback.Bag.back.MVP.png" alt="" />
      </div>
      <div class="card">
        <img
          src="assets/icons/images/Termos.preview.flera.MVP.png"
          alt="" />
      </div>
    </div>
  </div>

  <p class="description">
    Kinforma kombinerar techwear-estetik med <br />
    smarta, hållbara material. Design som utvecklas <br />
    med dig. Läs mer om vår idé.
  </p>

  <button class="cta-button button-animation">
    Mer om oss
    <svg class="icon">
      <use href="assets/icons/icons.svg#icon-see-more"></use>
    </svg>
  </button>

</div>
<footer>
  <!-- Desktop Layout -->
  <div class="footer-desktop">
    <div class="footer-left">
      <h2>BE THE FIRST. NOT THE REST.</h2>
      <p>
        Våra drops är begränsade – få förtur, previews och exklusiva
        erbjudanden.
      </p>
      <form class="email-form" id="newsletter">
        <input type="email" placeholder="exemple@kinforma.com" required />
        <button class="button-animation">
          Registrera
          <svg class="icon">
            <use href="assets/icons/icons.svg#icon-see-more"></use>
          </svg>
        </button>

      </form>
    </div>
    <div class="footer-right">
      <ul class="footer-links">
        <li>OM KINFORMA</li>
        <li>KONTAKTA OSS</li>
        <li>INTEGRITETSPOLICY</li>
        <li>COOKIEPOLICY</li>
      </ul>
      <div class="copyright">
        © 2025 Kinforma. Alla<br />rättigheter förbehållna.
      </div>
    </div>
  </div>

  <!-- Mobile Layout -->
  <div class="footer-mobile">
    <ul class="footer-links">
      <img src="assets/icons/sweden-flag.svg" class="language">
      <li><a href="#om">OM PROJEKTET</a></li>
      <li><a href="#kontakt">KONTAKTA OSS</a></li>
      <li><a href="#integritet">INTEGRITETSPOLICY</a></li>
      <li><a href="#cookie">COOKIEPOLICY</a></li>
    </ul>
    <input
      type="email"
      placeholder="exemple@kinforma.com"
      required
      class="mobile-email-submit" />
    <button type="submit" class="button-email-submit">Registrera</button>
    <div class="copyright">
      © 2025 Kinforma Alla rättigheter<br />förbehållna.
    </div>
  </div>

  <div class="logo-container">
    <div class="logo"><img src="/assets/icons/images/logowhite.png" /></div>
  </div>
</footer>